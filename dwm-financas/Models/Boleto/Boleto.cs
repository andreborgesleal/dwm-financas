...
namespace DWM.Models.Boleto
{
    ...
    /// </Author>
    public class BoletoDraw
    {
        EditarContaReceberViewModel cobranca = null;

        public BoletoDraw(int Banco, int operacaoId, int parcelaId)
        {
            boletoBancario = new BoletoBancario();
            boletoBancario.CodigoBanco = (short)Banco;
            cobranca = new EditarContaReceberViewModel() { operacaoId = operacaoId, parcelaId = parcelaId };
        }
        ...
        public string Caixa()
        {
            ...
            cobranca = factory.Execute(new ContaReceberEditarBI(), cobranca);
            EmpresaInterna x = new EmpresaInterna();

            DateTime vencimento = cobranca.dt_vencimento; 

            Cedente c = new Cedente("000.000.000-00", x.nome, "1234", "12345678", "9");

            c.Codigo = "112233";

            BoletoNet.Boleto b = new BoletoNet.Boleto(vencimento, cobranca.vr_principal, "2", cobranca.operacaoId.ToString("D10"), c);

            b.Sacado = new Sacado(cobranca.cpf_cnpj, cobranca.nome_cliente);
            b.Sacado.Endereco.End = cobranca.endereco + " " + cobranca.complemento;
            b.Sacado.Endereco.Bairro = cobranca.bairro;
            b.Sacado.Endereco.Cidade = cobranca.cidade;
            b.Sacado.Endereco.CEP = cobranca.cep;
            b.Sacado.Endereco.UF = cobranca.uf;
